<script lang="ts">
	...
	const { playerPos, onUpdate } = $props<{ playerPos: { x: number; z: number }; onUpdate: (pos: { x: number; z: number }) => void }>();
	...
	useTask((delta) => { tailRotation = Math.sin(Date.now() * 0.008) * 0.3; });
</script>
...
<T.Group bind:ref={whaleRef}>
	...
	<T.Mesh position={[0.65, 0.18, 0.22]}><T.SphereGeometry args={[0.08, 12, 8]} /><T.MeshBasicMaterial color="white" /></T.Mesh>
	<T.Mesh position={[0.66, 0.18, 0.25]}><T.SphereGeometry args={[0.04, 8, 6]} /><T.MeshBasicMaterial color="black" /></T.Mesh>
	<T.Mesh position={[0.65, 0.18, -0.22]}><T.SphereGeometry args={[0.08, 12, 8]} /><T.MeshBasicMaterial color="white" /></T.Mesh>
	<T.Mesh position={[0.66, 0.18, -0.25]}><T.SphereGeometry args={[0.04, 8, 6]} /><T.MeshBasicMaterial color="black" /></T.Mesh>
	...
	<T.Mesh position={[-0.1, -0.15, 0.45]} rotation={[0.4, 0, 0]} scale={[0.15, 0.06, 0.35]}>
		<T.BoxGeometry /><T.MeshStandardMaterial color="#1e40af" roughness={0.28} metalness={0.45} />
	</T.Mesh>
	<T.Mesh position={[-0.1, -0.15, -0.45]} rotation={[-0.4, 0, 0]} scale={[0.15, 0.06, 0.35]}>
		<T.BoxGeometry /><T.MeshStandardMaterial color="#1e40af" roughness={0.28} metalness={0.45} />
	</T.Mesh>
	<T.Mesh position={[-0.05, -0.25, 0]} scale={[0.8, 0.25, 0.55]}>
		<T.SphereGeometry args={[0.4, 16, 12]} /><T.MeshStandardMaterial color="#60a5fa" roughness={0.5} metalness={0.1} />
	</T.Mesh>
	<T.Mesh position={[0.3, 0.28, 0]} scale={[0.12, 0.08, 0.12]}>
		<T.CylinderGeometry args={[0.05, 0.08, 0.12, 8]} /><T.MeshStandardMaterial color="#172554" roughness={0.2} />
	</T.Mesh>
</T.Group>
